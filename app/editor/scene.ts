...
import type { Gradient, RobloxClass, SceneNode, Transition } from "./catalog";
...
export function duplicateSubtree(
  ...
  const nodes = subtree.map((n) => ({
    ...
    ...(n.gradient
      ? { gradient: { stops: n.gradient.stops.map((s) => ({ ...s })), rotation: n.gradient.rotation } }
      : {}),
...
const SLIDE_OFFSET = 0.2; // slide travel distance in scale units

// Body lines for a tweened show/hide of `targetVar`, emitted inside the button's
// Activated handler. show reveals then tweens to rest; hide tweens away then
// collapses on Completed (restoring the rest value so the next show is correct).
function tweenBody(
  targetVar: string,
  type: "show" | "hide",
  ...
  const ti = `local ti = TweenInfo.new(${fmt(transition.duration)}, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)`;
  const prop = transition.style === "scale" ? "Size" : "Position";
  const restPt = transition.style === "scale" ? target.size : target.pos;
  const rest = `UDim2.fromScale(${fmt(restPt.x)}, ${fmt(restPt.y)})`;

  if (transition.style === "scale") {
    const small = `UDim2.fromScale(${fmt(target.size.x * 0.5)}, ${fmt(target.size.y * 0.5)})`;
    return type === "show"
      ? [
          `${targetVar}.Visible = true`,
          `${targetVar}.${prop} = ${small}`,
          ti,
          `local tw = TweenService:Create(${targetVar}, ti, {${prop} = ${rest}})`,
          `tw:Play()`,
        ]
      : [
          ti,
          `local tw = TweenService:Create(${targetVar}, ti, {${prop} = ${small}})`,
  ...
  const dx = dir === "left" ? -SLIDE_OFFSET : dir === "right" ? SLIDE_OFFSET : 0;
  const dy = dir === "up" ? -SLIDE_OFFSET : dir === "down" ? SLIDE_OFFSET : 0;
  const off = `UDim2.fromScale(${fmt(target.pos.x + dx)}, ${fmt(target.pos.y + dy)})`;
  return type === "show"
    ? [
        `${targetVar}.Visible = true`,
        `${targetVar}.${prop} = ${off}`,
    ...
    : [
        ti,
        `local tw = TweenService:Create(${targetVar}, ti, {${prop} = ${off}})`,
        `tw.Completed:Connect(function()`,
        `\t${targetVar}.Visible = false`,
        `\t${targetVar}.${prop} = ${rest}`,
        `end)`,
...
function gradientColorSequence(grad: Gradient): string {
  const keypoints = grad.stops
    .map((s) => `ColorSequenceKeypoint.new(${fmt(s.at)}, ${color3(s.color)})`)
    .join(", ");
...
export function generateLuau(scene: SceneNode[]): string {
  ...
  if (rootGui && (rootGui.transparency < 1 || rootGui.gradient)) {
    ...
    if (rootGui.gradient) {
      ...
      out.push(`gui_bg_grad.Color = ${gradientColorSequence(rootGui.gradient)}`);
      out.push(`gui_bg_grad.Rotation = ${fmt(rootGui.gradient.rotation ?? 0)}`);
  ...
  const emit = (node: SceneNode, parentVar: string) => {
    ...
    out.push(`${v}.Name = ${luauString(node.name)}`);
    ...
    if (node.gradient) {
      ...
      out.push(`${v}_grad.Color = ${gradientColorSequence(node.gradient)}`);
      out.push(`${v}_grad.Rotation = ${fmt(node.gradient.rotation ?? 0)}`);
  ...
  if (
    emittedScene.some((n) => {
      const a = n.action;
      return (a?.type === "show" || a?.type === "hide") && !!a.transition;
    })
  ) {
    out.push('local TweenService = game:GetService("TweenService")');
  }
  ...
  for (const node of scene) {
    ...
    if (node.action.type === "hideGui") {
      body = ["gui.Enabled = false"];
    } else if (node.action.type === "remoteEvent") {
      const eventName = node.action.eventName;
      const binding = remoteBindings.find(
        (candidate) => candidate.eventName === eventName
      );
      if (binding) {
        body = [`${binding.variable}:FireServer(${luauString(node.action.argument)})`];
      }
      ...
      if (action && teleportPlaceIds.includes(action.placeId)) {
        body = [`teleportRequest:FireServer(${luauString(action.placeId)})`];
      }
    } else {
      ...
      if (targetVar) {
        if (
          ...
          body = tweenBody(targetVar, action.type, action.transition, target);
        } else {
          body = [
            action.type === "show"
              ? `${targetVar}.Visible = true`
